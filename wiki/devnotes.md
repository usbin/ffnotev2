...
## 최근 변경 (2026-06-04, 표시 모드 빈 줄 보존 + `<br/>` 줄바꿈)

- **읽기(표시) 모드에서 빈 줄 보존**: 마크다운 파서가 단락 사이 연속 빈 줄을 하나의 구분자로 소비해, Enter를 여러 번 쳐도 표시 모드 단락 간격이 항상 동일(`BuildParagraph` Margin 고정)했음. `MarkdownRenderer.Render`에서 각 top-level 블록의 `SourceSpan`으로 직전 블록과의 원본 빈 줄 수를 세어(`LineOfOffset` 헬퍼) 그만큼 `Margin.Top`에 `빈 줄 수 × fontSize×1.3`을 추가. soft break(단일 Enter=`LineBreak` 변환)와 일관되게 입력한 빈 줄이 그대로 간격으로 보임.
- **`<br/>` 줄바꿈 지원**: `AppendInline`에 `HtmlInline` 분기 추가 — `<br>`/`<br/>`/`<br />`를 `LineBreak`으로 변환. 표 셀 내부 등 Enter가 안 통하는 곳에서 명시적 줄바꿈 가능. 그 외 HTML 태그는 무시.
- **(보류) 편집 중 줄 맨 앞/맨 뒤 캐럿 비가시**: 재확인 시 현재는 캐럿이 보여 보류 처리. 재발 시 `TextEditor`(TextBox) 1px 캐럿이 콘텐츠 영역 좌/우 클립 경계에 겹쳐 잘리는 가능성부터 점검(좌우 `Padding`/내부 여백 조정).
...
## 최근 변경 (2026-05-17, 편집 모드 표 그리드 오버레이 완전 제거)

- **표 셀 그리드(테두리) 기능 제거**: 매 TextChanged/Scroll/SizeChanged마다 `OnRender`에서 전체 텍스트를 스캔해 `GetRectFromCharacterIndex`로 표 행을 다시 그리는 비용이 큰 노트에서 입력 지연을 유발. 사용자 요청으로 완전 제거 — `Controls/TableGridAdorner.cs` 삭제, `DraggableNoteControl`의 `_tableAdorner` 필드 / `AttachTableAdorner` / `DetachTableAdorner` / 3곳의 `InvalidateVisual` 호출 제거. 표 정렬(`AlignAllTablesInText`, 편집 종료 시 1회)·표시 모드 마크다운 표 렌더링은 그대로 유지.
