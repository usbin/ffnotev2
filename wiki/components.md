...
| Models/NoteBook.cs | `NoteBook` | Name, ProcessName, Notes/Groups 컬렉션, `SnapEnabled`(격자 스냅 토글, DB 영속), `OverlayDraft`(오버레이 초안, DB 영속, 키 입력마다 저장), `SortOrder`(사이드바 수동 정렬 순서, DB 영속 — 드래그 재정렬). `ObservableProperty Name/ProcessName/SnapEnabled/OverlayDraft` |
...
| 파일 | 클래스 | 핵심 메서드 |
|------|--------|-------------|
| Services/DatabaseService.cs | `DatabaseService` | `LoadAllNotebooks()`(SortOrder→Id 순 정렬), `SaveNoteItem()`, `UpdateNotePosition()`, `DeleteNoteItem()`, `SetNotebookSnapEnabled()`, `SetNotebookOverlayDraft()`, `UpdateNotebookOrder(orderedIds)`(드래그 재정렬 일괄 영속, 트랜잭션), `AddGroup()`/`UpdateGroup()`/`DeleteGroup()`. 새 노트북은 SortOrder=Id로 맨 아래. `InitializeSchema()`에 `NoteGroups` 테이블 + `Notebooks.SnapEnabled`/`OverlayDraft`/`SortOrder` 컬럼 ALTER 마이그레이션(SortOrder 초기값=Id) + `ColumnExists` 헬퍼 |
...
| 파일 | 설명 |
|------|------|
| Controls/DraggableNoteControl | 드래그 중 16ms `_dragTimer`로 마우스가 캔버스 가장자리(56px)면 `MainWindow.AutoPanForDrag` 호출해 화면 자동 스크롤 + 노트가 커서 따라 새 영역으로 이동(`ApplyDrag()`는 `Mouse.GetPosition(canvas)` 기준 — MouseMove/타이머 공용). 타이틀바 드래그로 이동(조건부 격자 스냅, 선택된 모든 노트 동기 이동) + **선택된 상태(`IsSelected=true`)에선 본문 어디서든 드래그**(`BodyDrag_*` 핸들러, 4px 임계값 초과 시 드래그로 승격, 미만이면 단순 클릭/더블클릭/하이퍼링크 그대로 동작). 첫 클릭으로 막 선택된 노트는 그 클릭으로는 본문 드래그 X(`_clickStartedSelected` 플래그). 편집 중(TextEditor visible)에는 본문 드래그 비활성. 선택 시 본문 영역에 SizeAll 커서(DataTrigger), 4 외곽(상/하/좌/우 5px) + 4 코너(10×10 투명, 대각선) Thumb로 리사이즈(누적 델타 + 조건부 스냅, 좌/상은 X/Y와 W/H 동시 변경, 선택된 모든 노트 동기 변경, min 80×40, 단일 핸들러가 `sender.Tag`(`Top/Bottom/Left/Right/TopLeft/TopRight/BottomLeft/BottomRight`)로 분기), 타입별 내용 표시(**텍스트는 FlowDocumentScrollViewer↔TextBox 스왑** — 표시 모드에서 마크다운 렌더링 + 컬러 이모지 + 이미지 임베드, 편집 모드에서 raw 마크다운 입력, IME 컨텍스트 버그 회피 위해 컨트롤 분리/이미지/Hyperlink), 더블클릭 또는 자동(`IsEditing=true`)으로 텍스트 편집 진입, DataContext PropertyChanged 구독으로 외부 IsEditing 변화 + Content 변경(표시 모드일 때만 RefreshDocument) 감지, `SettingsService.SettingsChanged` 구독으로 폰트 변경 즉시 반영, TextBox는 `UpdateSourceTrigger=PropertyChanged`로 키 입력 즉시 `Content` 갱신, **편집 중 Ctrl+V로 클립보드 이미지 → `%APPDATA%\ffnotev2\images\{guid}.png` 저장 + `![](파일명)` 마크다운 캐럿 삽입** (텍스트만 있으면 양보), `PreviewMouseLeftButtonDown`에서 클릭 소스가 TextBox가 아니면 `MainWindow.FocusCanvas()`로 다른 노트의 LostFocus 트리거(커서 잔존 방지) + Shift 토글/단일 선택 처리, `IsSelected`에 따라 외곽 Border 파란 테두리(DataTrigger). 자체 우클릭 핸들러 없음 — 우클릭은 캔버스로 버블링되어 통합 메뉴에서 "삭제하기" 항목 추가. 헤더 × 버튼 없음 |
...
| MainWindow | 전체 레이아웃 (토글 가능 사이드바 + Pan/Zoom 캔버스), 사이드바 ◀/▶ 토글 + 노트북 추가/폰트 설정 버튼, Ctrl+V 붙여넣기(world 변환), **통합 우클릭 컨텍스트 메뉴** (`Canvas_MouseRightButtonUp`): 모든 우클릭이 여기로 버블링 → 항상 "새 텍스트 노트", 선택된 노트/그룹 있으면 "그룹 만들기/해제 (N개)", 우클릭 대상이 노트면 "삭제하기", 비선택 그룹이면 "그룹 해제" 추가. 우클릭 드래그는 노트/그룹 위에서도 팬 — `_rightClickOrigin`을 down 시 저장해 캔버스 캡처 후에도 메뉴 대상 식별 가능. `_panMoved=true`면 메뉴 억제. 휠클릭/우클릭 드래그 → 팬, **빈 캔버스 좌클릭 드래그 → 마키 선택** (인터섹트, 노트+그룹 모두 선택), 단순 좌클릭/Esc → 선택 해제, **Ctrl+G 그룹 만들기 / Ctrl+Shift+G 그룹 해제**, **Delete → 선택 노트/그룹 모두 삭제**, **Alt+화살표(편집 중) 또는 화살표(비편집·단일 선택) → 인접 노트 이동**, Ctrl+휠 → 마우스 앵커 줌(0.2x~4.0x), 캔버스 콘텐츠는 `Grid CanvasContent` 하나로 묶여 RenderTransform 공유 — 그룹 ItemsControl(뒤) + 노트 ItemsControl(앞, **`Canvas Background="{x:Null}"` 필수** — 뒤 그룹 hit-test 통과용), 우하단 인디케이터: **미니맵 토글 🗺** + 일괄 정렬 ⊟ + 격자 스냅 토글(노트북별) ⊞ + 줌 % + 뷰 초기화, 글로벌 단축키 등록, 닫기 시 트레이로 숨김. `IsOriginInsideNote`는 `DraggableNoteControl`+`GroupBoxControl` 모두 검사(마키/팬 가드). **사이드바 노트북 드래그 재정렬**: `NotebookList_PreviewMouseLeftButtonDown/Move`(4px 임계+`DragDrop.DoDragDrop`), `DragOver`에서 `InsertionLineAdorner` 삽입선 표시, `Drop`에서 `ComputeDropIndex` 후 `MainViewModel.MoveNotebook` (⋯ 버튼 위 클릭은 드래그 제외). **미니맵**(`MinimapToggle_Click`): 기본 숨김, 토글 시 120ms 타이머로 `RefreshMinimap()` — 전체 노트/그룹 + 현재 뷰포트 bbox를 미니맵 캔버스(240×170)에 축소, 노트는 타입별 색 사각형·그룹은 외곽선·뷰포트는 파란 사각형. 미니맵 클릭/드래그 → `CenterViewOnMinimapPoint`로 해당 월드 좌표를 화면 중앙으로 이동. `AutoPanForDrag(p)` — 가장자리 자동 스크롤(public, DraggableNoteControl이 호출) |
...
| Dialogs/TableEditorDialog | 텍스트 노트 편집 중 `Ctrl+E`로 호출(캐럿이 표 행 안일 때). WPF 순정 `DataGrid` + `DataTable` 모델 — 셀 편집, 행/열 추가·삭제, 컬럼 헤더 더블클릭으로 이름 변경(`PreviewMouseLeftButtonDown`+`ClickCount==2`로 처리 — 리오더/리사이즈와 공존). `CanUserAddRows=False`(placeholder 행 숨김). 컬럼 추가/삭제/이름변경 후 `RebindGrid()`로 ItemsSource 재설정해 즉시 반영(DataGrid는 DataTable 컬럼 변경을 자동 재생성 안 함). OK 시 `ResultMarkdown`이 헤더+separator+본문 행 형식의 마크다운 표 문자열 반환. `DraggableNoteControl.EditTableAtCaret`이 표 영역을 식별해 결과로 치환 |
...
- 캔버스: 휠클릭/우클릭 드래그(팬) / `Ctrl+휠`(줌) / 노트를 가장자리로 드래그 시 화면 자동 스크롤
- 사이드바: 노트북 드래그로 순서 변경(DB 영속) / 우하단 🗺 미니맵 토글 — 전체 노트 영역 + 현재 화면 표시, 미니맵 클릭·드래그로 화면 이동
- 표 편집: `Ctrl+E`(캐럿이 표 안) → DataGrid 다이얼로그
